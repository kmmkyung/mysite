...
****************************/
    ...
    // 유효성검사 기준: 전송시 아이디 비번 모두 빈값 없어야 함
    // 이벤트 대상: #sbtn
    // 이벤트 종류: click
    $("#sbtn").click(function (e) {
        ...
        console.log("아이디", groSpace(mid.val()));
        console.log("비번", groSpace(mpw.val()));

        // 유효성 검사하기
        // 아이디, 비번 중 하나라도 비어 있으면 불통과(if)
        // if : 불통과시 / else : 통과시
        if (mid.val().trim() === "" || mpw.val().trim() === "") {
            alert("아이디, 비밀번호 모두 입력하시오");
            // 초기화+아이디에 포커스
            mid.val("").focus();
            mpw.val("");
        } else {
